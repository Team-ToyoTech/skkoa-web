...
`입력`은 정수, 논리, 실수, 문자, 포인터 값을 공백이나 줄바꿈으로 구분해 순서대로 읽는다. 문자열 변수는 다음 줄 전체를 읽으므로 문자열 안의 띄어쓰기를 보존한다.
...
온라인 컴파일러의 Console 입력창도 같은 규칙으로 공백과 줄바꿈이 섞인 입력을 한 번에 받을 수 있다.
