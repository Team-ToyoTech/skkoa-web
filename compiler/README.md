...
SKKOA; LTW는 **Starter Kit with Korean Oriented Architecture; Language to Write**를 뜻하며, 한국어 기반 명령어와 직관적인 구조를 사용하는 교육용 프로그래밍 언어이다.
...
통합 설치 파일을 사용할 경우 C++17 컴파일러, NASM, GCC/Clang은 설치 파일이 자동으로 준비된다. 수동 빌드 때만 직접 도구 설치가 필요하다.
...
Windows에서는 메인 화면 Download 버튼이 `skkoa-windows.exe`를 기본으로 내려받는다. 설치 파일은 필요한 도구를 함께 준비하므로 별도로 C++ 컴파일러, NASM, GCC를 미리 설치하지 않아도 된다. Linux에서는 아래처럼 설치 스크립트를 받을 수 있다.
...
온라인 컴파일러의 Console 입력창도 같은 규칙으로 공백과 줄바꿈이 섞인 입력을 한 번에 처리한다.
...
- 문자열 리터럴 출력
- 정수/논리/실수/문자/문자열/포인터 입력, 공백/줄바꿈 기반 묶음 입력
