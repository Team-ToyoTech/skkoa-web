...
void CodeGenerator::emitInput(const InputStmt &statement) {
    ...
    if (statement.index) {
        emitArrayAddress(statement.name, *statement.index);
        text_ << "    mov rsi, rax\n";
        text_ << "    lea rdi, [fmt_scan_int]\n";
    } else {
        text_ << "    lea rsi, [rbp - " << slot.offset << "]\n";
        if (slot.type.base == ValueType::Float) {
            text_ << "    lea rdi, [fmt_scan_float]\n";
        } else if (slot.type.base == ValueType::Char) {
            text_ << "    lea rdi, [fmt_scan_char]\n";
        } else {
            text_ << "    lea rdi, [fmt_scan_int]\n";
        }
    }
...
void CodeGenerator::emitExpr(const Expr &expression) {
    if (auto *literal = dynamic_cast<const IntLiteralExpr *>(&expression)) {
        ...
        text_ << "    call " << functionLabels_.at(call->name) << "\n";
    }
}
...
void CodeGenerator::emitStringConcat() {
    ...
    text_ << "    push rbx\n";
    text_ << "    sub rsp, 40\n";
    text_ << "    mov [rbp - 16], rdi\n";
    text_ << "    mov [rbp - 24], rsi\n";
    text_ << "    mov rbx, rdi\n";
    text_ << "    xor rcx, rcx\n";
    ...
    text_ << "    mov [rbp - 32], rcx\n";
    text_ << "    mov rbx, [rbp - 24]\n";
    text_ << "    xor rcx, rcx\n";
    ...
    text_ << "    mov rdi, [rbp - 32]\n";
    text_ << "    add rdi, rcx\n";
    text_ << "    add rdi, 1\n";
    text_ << "    call malloc\n";
    text_ << "    mov [rbp - 40], rax\n";
    text_ << "    mov rdi, rax\n";
    text_ << "    mov rsi, [rbp - 16]\n";
