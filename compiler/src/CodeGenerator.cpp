...
string CodeGenerator::generateAssembly(Program &program,
    ...
    text_ << "extern " << externalSymbol("strlen") << "\n";
    text_ << "extern " << externalSymbol("strcmp") << "\n\n";
    text_ << "section .text\n";
    emitStringConcat();
    text_ << "\nskkoa_substring:\n";
    text_ << "    push rbp\n";
    ...
    text_ << "    add rsi, [rbp - 40]\n";
    text_ << "    mov rdi, rax\n";
    text_ << "    xor rcx, rcx\n";
    text_ << ".skkoa_substring_loop:\n";
    text_ << "    cmp rcx, qword [rbp - 48]\n";
    text_ << "    jge .skkoa_substring_done\n";
    text_ << "    mov al, [rsi + rcx]\n";
    text_ << "    cmp al, 0\n";
    text_ << "    je .skkoa_substring_done\n";
    text_ << "    mov [rdi + rcx], al\n";
    text_ << "    inc rcx\n";
    text_ << "    jmp .skkoa_substring_loop\n";
    text_ << ".skkoa_substring_done:\n";
    text_ << "    mov byte [rdi + rcx], 0\n";
...
void CodeGenerator::prepareStructLayouts(Program &program) {
    ...
    for (const auto &structure : program.structs) {
        ...
        layout.size = offset;
        structLayouts_[structure->name] = layout;
    }
}
        ...
        if (arrayLiteral) {
            for (size_t i = 0; i < arrayLiteral->elements.size(); i++) {
                auto index = make_unique<IntLiteralExpr>(
                    statement.location, static_cast<long long>(i));
                emitArrayAddress(statement.name, *index);
                text_ << "    push rax\n";
                if (slot.type.base == ValueType::Float) {
                    emitFloatExpr(*arrayLiteral->elements[i]);
...
void CodeGenerator::emitFieldAssignment(const FieldAssignmentStmt &statement) {
    ...
    if (field && field->type.base == ValueType::Float) {
        emitFloatExpr(*statement.value);
        text_ << "    pop rbx\n";
        text_ << "    movsd [rbx], xmm0\n";
    } else {
...
void CodeGenerator::emitExpr(const Expr &expression) {
    if (auto *literal = dynamic_cast<const IntLiteralExpr *>(&expression)) {
        ...
        if (call->name == "비교") {
            emitExpr(*call->arguments[0]);
            text_ << "    push rax\n";
            emitExpr(*call->arguments[1]);
            text_ << "    mov " << registers[1] << ", rax\n";
            text_ << "    pop " << registers[0] << "\n";
